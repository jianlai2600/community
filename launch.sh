...
function run() {
  ...
  nohup java -server -Dspring.devtools.restart.enabled=false -Xms512m -Xmx512m -Xmn256m -XX:NativeMemoryTracking=detail -XX:-OmitStackTraceInFastThrow -jar ${JAR_NAME} > /dev/null 2>&1 &
  echo $! 1> pid.log
}
